...
import { draftApplicationEmail } from "@/services/aiService";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { jobId, tone = "formal", recipientName, recipientEmail } = await req.json();
    ...
    const email = await draftApplicationEmail(ctx, job, profile, {
      name: recipientName ?? job.hiring_manager ?? null,
