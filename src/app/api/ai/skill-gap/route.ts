...
import { detectSkillGaps } from "@/services/aiService";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const skillGaps = await detectSkillGaps(ctx, job, profile);
    await supabase.from("jobs").update({ skill_gaps: skillGaps }).eq("id", jobId);
    return NextResponse.json({ success: true, skillGaps });
  } catch (err) {
