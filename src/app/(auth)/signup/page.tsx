...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    if (data.user && !data.session) {
      ...
      toast.success("Welcome to JobRadar!");
    ...
    <div className="min-h-screen flex items-center justify-center bg-signal-bg bg-signal-gradient bg-grid p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Start finding jobs that match you</p>
        </div>
        ...
        <div className="space-y-2 mb-6">
          ...
          <button onClick={() => handleOAuth("github")} disabled={loading !== null}
            className="w-full flex items-center justify-center gap-3 rounded-xl border border-white/[0.08] bg-signal-surface px-4 py-3 text-sm font-medium text-gray-200 hover:bg-signal-card-hover transition-all disabled:opacity-50">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs"><span className="px-3 bg-signal-bg text-gray-600">or with email</span></div>
        </div>
        ...
        <form onSubmit={handleEmailSignup} className="space-y-3">
          <div className="relative">
            ...
            <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name"
              className="w-full rounded-xl border border-white/[0.08] bg-signal-surface pl-10 pr-4 py-3 text-sm text-gray-200 placeholder-gray-600 focus:border-signal-cyan/40 focus:outline-none" />
          </div>
          <div className="relative">
        ...
        <p className="mt-8 text-center text-sm text-gray-500">
          ...
          <Link href="/login" className="text-signal-cyan hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
