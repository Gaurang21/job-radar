...
import type { Job, CoverLetterTone } from "@/types";
...
const TONES: { id: CoverLetterTone; label: string; desc: string }[] = [
  ...
  { id: "concise", label: "Concise", desc: "Direct, under 250 words" },
];
...
export default function CoverLetterModal({ job, onClose }: Props) {
  ...
  const generate = async (newTone: CoverLetterTone) => {
    ...
    try {
      const res = await fetch("/api/ai/cover-letter", {
        ...
        body: JSON.stringify({ jobId: job.id, tone: newTone }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (data.code === "ANTHROPIC_ERROR") toast.error("⚠️ AI unavailable — check API key");
  ...
  const handleDownload = async () => {
    try {
      ...
      const paragraphs = content.split("\n\n").filter(Boolean).map(
        (text) => new Paragraph({ children: [new TextRun({ text, size: 24, font: "Calibri" })], spacing: { after: 200 } })
      );
      ...
      a.href = url; a.download = `Cover_Letter_${job?.company?.replace(/\s+/g, "_")}.txt`; a.click();
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            {!isGenerating && content && (
              <>
                <button onClick={handleCopy} className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-gray-200 transition-all">
                  {copied ? <CheckCircle className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                  {copied ? "Copied!" : "Copy"}
                </button>
                <button onClick={handleDownload} className="flex items-center gap-1.5 rounded-lg border border-signal-cyan/30 bg-signal-cyan/10 px-3 py-1.5 text-xs font-medium text-signal-cyan hover:bg-signal-cyan/20 transition-all">
